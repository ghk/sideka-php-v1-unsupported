...
function edit_kewarganegaraan(id) {
  window.location = _base_url + 'pustaka/c_kewarganegaraan/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'pustaka/c_kewarganegaraan/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("pustaka/c_kewarganegaraan/delete/");?>",
					   data: "items="+itemlist,
