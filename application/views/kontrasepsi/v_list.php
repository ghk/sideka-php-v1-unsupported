...
function edit_kontrasepsi(id) {
  window.location = _base_url + 'pustaka/c_kontrasepsi/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'pustaka/c_kontrasepsi/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("pustaka/c_kontrasepsi/delete/");?>",
					   data: "items="+itemlist,
