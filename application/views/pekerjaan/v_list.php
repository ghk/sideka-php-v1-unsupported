...
function edit_pekerjaan(id) {
  window.location = _base_url + 'datapenduduk/c_pekerjaan/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'datapenduduk/c_pekerjaan/add';
    }	
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("datapenduduk/c_pekerjaan/delete/");?>",
					   data: "items="+itemlist,
