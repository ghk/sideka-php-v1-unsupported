...
function edit_apbdes(id) {
  window.location = _base_url + 'apbdes/c_apbdes/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'apbdes/c_apbdes/add';
    }	
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("apbdes/c_apbdes/delete/");?>",
					   data: "items="+itemlist,
