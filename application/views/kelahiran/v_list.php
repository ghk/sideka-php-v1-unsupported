...
function edit_kelahiran(id) {
  window.location = _base_url + 'peristiwa/c_kelahiran/edit/' + id;
}

function cetak(id_kelahiran) {
  $("#MyModalBody").html('<iframe src="<?php echo base_url();?>peristiwa/c_kelahiran/cetakById/'+id_kelahiran+'" width="100%" height="350" frameborder="no"></iframe>');
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'peristiwa/c_kelahiran/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("peristiwa/c_kelahiran/delete/");?>",
					   data: "items="+itemlist,
  ...
  <div class="modal-dialog" style="width: 90%;">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel" ><span class="fa fa-print"></span>&nbsp;Pencetakan Surat Kelahiran</h4>
      </div>
