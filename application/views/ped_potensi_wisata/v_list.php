...
function edit_ped_potensi_wisata(id) {
  window.location = _base_url + 'admin/c_ped_potensi_wisata/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'admin/c_ped_potensi_wisata/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("admin/c_ped_potensi_wisata/delete/");?>",
					   data: "items="+itemlist,
