...
function edit_status_kawin(id) {
  window.location = _base_url + 'pustaka/c_status_kawin/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'pustaka/c_status_kawin/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("pustaka/c_status_kawin/delete/");?>",
					   data: "items="+itemlist,
