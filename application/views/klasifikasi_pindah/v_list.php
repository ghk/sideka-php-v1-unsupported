...
function edit_klasifikasi_pindah(id) {
  window.location = _base_url + 'pustaka/c_klasifikasi_pindah/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'pustaka/c_klasifikasi_pindah/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("pustaka/c_klasifikasi_pindah/delete/");?>",
					   data: "items="+itemlist,
