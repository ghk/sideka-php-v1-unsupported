...
function edit_pekerjaan_ped(id) {
  window.location = _base_url + 'datapenduduk/c_pekerjaan_ped/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'datapenduduk/c_pekerjaan_ped/add';
    }	
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("datapenduduk/c_pekerjaan_ped/delete/");?>",
					   data: "items="+itemlist,
