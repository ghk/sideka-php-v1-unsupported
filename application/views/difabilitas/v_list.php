...
function edit_difabilitas(id) {
  window.location = _base_url + 'pustaka/c_difabilitas/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'pustaka/c_difabilitas/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("pustaka/c_difabilitas/delete/");?>",
					   data: "items="+itemlist,
