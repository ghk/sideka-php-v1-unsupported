...
function edit_pindah_keluar(id) {
  window.location = _base_url + 'peristiwa/c_pindah_keluar/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'peristiwa/c_pindah_keluar/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("peristiwa/c_pindah_keluar/delete/");?>",
					   data: "items="+itemlist,
